    ...
    public class AdjustInfluence : PlayerAction
    {
        ...
        public override async Task Action()
        {
            ...
            if (influencePool != 0)
            {
                ...
                {
                    TwilightStruggle.UI.Message.SetMessage($"{(influencePool > 0 ? "Add" : "Remove")} {(influencePool > 0 ? Player : Player.Enemy).name} Influence ({Mathf.Abs(influencePool)} remaining)");
